...
# --- 回帰係数 (流路長ごとに固定値)
st.subheader("流路長ごとの回帰係数 a, b, c（固定値）")

# 流路長ごとに固定値を設定（必要に応じて変更可能）
coeff_dict = {
    200: (0.053609639, -0.143441753, 0.568830078),
    400: (0.113674295, -0.397983625, 0.73875903),
    600: (0.086738435, -0.222033841, 0.484408535),
    800: (0.177464675, -0.770306058, 1.311102984),
}

# 表形式で表示（確認用）
coeff_df = pd.DataFrame.from_dict(coeff_dict, orient="index", columns=["a", "b", "c"])
coeff_df.index.name = "Length (mm)"
